...
def craiglockhart_and_tollcross
  ...
  JOIN
    routes b ON (a.company = b.company AND a.num = b.num)
  WHERE
    a.stop_id = 53 AND b.stop_id = 230 --better way?
  SQL
end
...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
    SELECT
      all_stops.name, routes.company, routes.num
    FROM
      routes
    JOIN
      routes b ON (routes.company = b.company AND routes.num = b.num)
    JOIN
      stops ON stops.id = routes.stop_id
    JOIN
      stops all_stops ON all_stops.id = b.stop_id
    WHERE
      stops.id = 53
  SQL
end
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    SELECT DISTINCT
      a.num, a.company, stops_a.name, d.num, d.company
    FROM
      routes a
    JOIN
      routes b ON (a.company = b.company AND a.num = b.num)
    JOIN
      stops stops_a ON (b.stop_id = stops_a.id)
    JOIN
      routes c ON (stops_a.id = c.stop_id)
    JOIN
      routes d ON (c.company = d.company AND c.num = d.num)
    JOIN
      stops stops_b ON (d.stop_id = stops_b.id)
    WHERE
